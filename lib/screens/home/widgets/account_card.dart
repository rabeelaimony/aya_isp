...
class _DueSoonReminder extends StatelessWidget {
  final Animation<double> animation;
  final int remainingDays;

  const _DueSoonReminder({
  ...
  Widget build(BuildContext context) {
    final theme = Theme.of(context);
    final daysLabel = remainingDays <= 0 ? 'اليوم' : '$remainingDays يوم';

    return AnimatedBuilder(
      animation: animation,
      builder: (_, __) {
        final glow = 8 + (animation.value * 10);
